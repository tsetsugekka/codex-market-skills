...
- `https://daytrading.monster/api/gamma/`: SPX/SPXW summary, expiry structure, levels and 0DTE five-minute context. `https://daytrading.monster/api/camarilla/nikkei` and `https://daytrading.monster/api/camarilla/sse`: fixed-index Camarilla, gaps and available five-minute/MA/VWAP context. Use `price_trajectory.points` (`timestamp`, `index_value`) for all available source price points in the returned session; it is price-only, not interpolated or guaranteed tick-complete. Match the market and session; missing current-session trajectories stay unavailable.
