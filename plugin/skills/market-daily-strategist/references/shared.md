...
- If using the optional narrative-status helper, treat it as a theme pre-screen. Account/source quality can be assumed acceptable for screening, but timeliness must still be checked: API `generated_at`/`reviewed_at` and entry `time` must fit the report window. Fresh entries can suggest which themes to verify; stale entries are background only and must not drive current-session strategy.
...
- For U.S. and Japan market analysis, treat the current DayTrading.monster news-details index as the most important public theme-discovery source. Use relevant linked details and X account posts before deciding which themes, sectors, or movers deserve deeper verification.
- For broad theme discovery, the optional narrative-status helper filters the current news-details index across AI, geopolitics, commodities, rates/bonds, index/gamma, Japan, China, non-AI sectors, and crypto. It does not replace the same-day news scan or price/sector confirmation.
- Read `https://daytrading.monster/api/24hfeed/details` for the latest 120-hour, at-most-100 news index (`title`, `url`, `time`, `category`, `category_label`); open the selected detail URLs when content is needed. Read `https://daytrading.monster/api/24hfeed/x-monitor` for social-media-derived posts within its explicit eight-hour snapshot window. Verify decisive items with live news, original/source reporting when available, and prices. Use current documented APIs, not historical/archive feed files or static SEO summaries.
...
- For overall Japan PTS rising-mover analysis, prefer `https://daytrading.monster/api/pts/model1` (day), `https://daytrading.monster/api/pts/model2` (after close), or `https://daytrading.monster/api/pts/model3` (night), selecting the session relevant to the report. Each supplies `ranking` and `latest_upward_reason_summary`; check `source_updated_at` and reason timestamps. For other Japanese rankings, including PTS declines, use the appropriate options at `https://kabutan.jp/warning/` and `https://finance.yahoo.co.jp/stocks/ranking/up`; verify the selected ranking's session, direction and time.
...
## DTM Research API Routing

Use the public contracts in `https://daytrading.monster/api-docs/`. Research responses contain JSON even when served as `text/plain`; use only documented parameters and current snapshot timestamps. An unavailable or stale response is a missing layer, not a reason to read old frontend JSON files.

- `https://daytrading.monster/api/themes`: read across US, JP and CN for theme, supply-chain and cross-market transmission; connect the affected markets before focusing on the report's market. `constituents` includes `weight` and `reason_zh`; stage returns and `price_dates` are not intraday quotes. Do not scan all constituents merely because they are available.
- `https://daytrading.monster/api/chinastock-anomaly`: for A-share theme strength, limit-up concepts, current anomaly reasons and limit-up rows; check each section's trading date and time, including hourly ranking snapshots.
- `https://daytrading.monster/api/gamma/`: SPX/SPXW summary, expiry structure, levels and 0DTE five-minute context. `https://daytrading.monster/api/range/nikkei` and `https://daytrading.monster/api/range/sse`: index trajectories with fixed `camarilla` levels, `chart_range`, range-filtered `daily_gaps`, `trajectory_5m`, and actually returned `moving_averages`/`vwap`. Check `market_date`, `market_status` and `stale`; these two endpoints contain no Gamma, GEX, DEX, expiry or options fields. Use `price_trajectory.points` (`timestamp`, `index_value`) for all available source price points in the returned session; it is price-only, not interpolated or guaranteed tick-complete. Match the market and session; an empty premarket trajectory stays unavailable, never replaced with the previous session. HTML contracts: `https://daytrading.monster/api-docs/range-nikkei` and `https://daytrading.monster/api-docs/range-sse`.
- This strategy skill does not request DTM ratings APIs. A separate stock-specific move-reason workflow may read its own market's ratings when relevant.
...
- Prefer `us-stock-gamma-moomoo` for US index gamma/options-wall work when available; if necessary data or computation capability is unavailable, mark the value unavailable instead of substituting stale public screenshots, and optionally suggest setup only if the user needs that layer.
...
When the report needs market-regime, mainline, or bull/bear structure judgment, combine current data with the `mainline x funds x game x cycle` framework from [global-mainline-funds-game-cycle](../../../../skills/market-daily-strategist/references/global-mainline-funds-game-cycle.md).
