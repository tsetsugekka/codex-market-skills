...
- kabutan.jp
- finance.yahoo.co.jp
- DTM PTS model APIs, using the session mapping in `shared.md`
- トレーダーズ・ウェブ
- みんかぶ
- `日経先物最新`
- `PTS急騰銘柄`
...
- PTS sharp risers/fallers as the primary stock-discovery layer. For the overall PTS rising picture, prioritize `https://daytrading.monster/api/pts/model3` for the relevant night session; select model1 or model2 for a matching day/after-close session. For declines and other ranking options, use the Kabutan/Yahoo entrypoints in `shared.md`. Check snapshot times and use company/news searches after the PTS list to explain why the stock moved.
