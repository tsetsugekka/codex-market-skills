...
- DTM PTS model APIs for after-close PTS rising movers, using the session mapping in `shared.md`. In output prose, call this layer `PTS异动` or `PTS名单`; do not name the site/source by default. If no relevant PTS mover changes the conclusion, omit the layer instead of writing negative filler.
...
- Use PTS as the main after-close stock-discovery layer. Prefer `https://daytrading.monster/api/pts/model2` for overall after-close rising movers, or `https://daytrading.monster/api/pts/model3` once the relevant night session is available; check `source_updated_at`. For declines and other rankings, use the Kabutan/Yahoo entrypoints in `shared.md`. After identifying PTS risers/fallers, search company disclosures and news to explain the move instead of trying to discover every mover from general news first.
...
Start with a concise conclusion; the following title/section layout is a reusable example, subject to the user or Task format.
