...
- For SPX/Nikkei special workflows, default to a concise human text summary. Do not generate the old simple text-only SPX HTML report; use JSON only when the user explicitly requests raw export, and use a separate chart dashboard workflow when the user asks for visual HTML. Unfinished files should not appear as incidental artifacts.
...
- 2026-05-22: Removed the simple text-only SPX HTML report path from `spx_intraday_latest.py`; visual SPX HTML should use the chart dashboard workflow instead.
