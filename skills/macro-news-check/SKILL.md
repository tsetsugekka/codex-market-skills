...
4. `DayTrading.monster 24H Feed` (`https://daytrading.monster/24hfeed/`): usable as a public current-page social-media layer for global macro, AI, rates/bonds, US index/gamma, Japan, China, commodities, non-AI sectors, and crypto. For broad U.S. or Japan market context it is especially useful for theme discovery; for macro checks, use it as an auxiliary source to generate candidates that still need live tape and price confirmation. Use only the current public snapshots:
   - `Market narrative monitor`: `https://daytrading.monster/24hfeed/feed24.json` exposes current per-topic entries, timestamps, directions, and evidence links.
   - `X account monitor`: `https://daytrading.monster/24hfeed/x-monitor-feed.json` exposes the current public account snapshot. Do not read `accounts.json`, per-account `days/*.json`, `narrative_history`, or legacy `market_recent.json` / `market_tweets.json` for market analysis.
...
If a market report passes in pre-screened `feed24.json` entries, or if you fetch the current DayTrading.monster 24H Feed snapshots directly, use them only as social-media-derived macro or theme candidates. Use only entries whose source/update time fits the active market window. Do not mine backfilled rows in the snapshot as historical evidence. Fresh entries may guide what to verify in live macro tape and market prices; stale entries must be ignored.
...
2. Fetch the minimum needed recent items from both streams:
   ...
   - Use the current DayTrading.monster 24H Feed snapshots as an extra narrative pre-screen when the question is broad macro, cross-asset, pre-market, after-hours, or market-regime related. Prefer `feed24.json` for current topic summaries and `x-monitor-feed.json` only when raw account context materially matters. Do not use `accounts.json`, daily shards, narrative history, legacy account files, HTML `noscript`, or static SEO text as current evidence. Do not use 24H Feed alone as final evidence. In final prose, describe this layer generically as `当前叙事预筛` or `当前主题线索`, not by site/feed/page name.
