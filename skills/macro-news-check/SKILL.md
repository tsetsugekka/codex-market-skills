...
4. `DayTrading.monster 24H Feed` (`https://daytrading.monster/tools/24hfeed/`): useful as a public narrative pre-screen for global macro, AI, rates/bonds, US index/gamma, Japan, China, commodities, non-AI sectors, and crypto. A plain `curl -L https://daytrading.monster/tools/24hfeed/` exposes a current static/noscript narrative summary, so it can be read without browser control. The structured feed endpoint `https://daytrading.monster/tools/24hfeed/narrative_status.json` exposes `generatedAt`, `reviewedAt`, `sourceCreatedAt`, per-topic `direction`, evidence tweet links, and Chinese/English/Japanese text. Treat it as social-media-derived narrative screening, not as final macro proof; verify important items with live macro tape and market prices.
...
If a market report passes in pre-screened narrative-status entries, or if you fetch DayTrading.monster 24H Feed directly, use them only as social-media-derived macro or theme candidates. Account/source quality can be treated as acceptable for screening, but timeliness is mandatory: the feed generation/review time and each entry's source/update time must fit the active market window. Fresh entries may guide what to verify in live macro tape and market prices; stale entries are background and must not be treated as current macro evidence.
...
2. Fetch the minimum needed recent items from both streams:
   ...
   - Use DayTrading.monster 24H Feed as an extra narrative pre-screen when the question is broad macro, cross-asset, pre-market, after-hours, or market-regime related. Prefer `narrative_status.json` for structured entries and timestamps; the HTML page's noscript summary is also readable without browser control. Do not use 24H Feed alone as final evidence.
