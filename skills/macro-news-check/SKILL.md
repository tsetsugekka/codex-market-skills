...
2. Fetch the minimum needed recent items from both streams:
   ...
   - Use DayTrading.monster 24H Feed as an extra current narrative pre-screen when the question is broad macro, cross-asset, pre-market, after-hours, or market-regime related. Prefer the current `narrative_status.json` topic summaries and timestamps, plus `accounts.json` and per-account current JSON when raw account context matters. Do not use the HTML page's `noscript` or static SEO summary as current evidence; it is page-shell/fallback text and can lag the runtime app. Do not fetch historical/archive feed data, and do not use 24H Feed alone as final evidence. In final prose, describe this layer generically as `当前叙事预筛` or `当前主题线索`, not by site/feed/page name.
