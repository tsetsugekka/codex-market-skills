...
4. `DayTrading.monster 24H Feed` (`https://daytrading.monster/tools/24hfeed/`): usable as a public current-page social-media layer for global macro, AI, rates/bonds, US index/gamma, Japan, China, commodities, non-AI sectors, and crypto. For broad U.S. or Japan market context it is especially useful for theme discovery; for macro checks, use it as an auxiliary source to generate candidates that still need live tape and price confirmation. The page has two distinct monitors:
   - `Market narrative monitor`: `https://daytrading.monster/tools/24hfeed/narrative_status.json` exposes `generatedAt`, `reviewedAt`, current per-topic `text`/`direction`, source timestamps, and supporting X evidence links.
   - `X account monitor`: `https://daytrading.monster/tools/24hfeed/accounts.json` lists monitored accounts and their freshness. Per-account current posts are available under paths such as `https://daytrading.monster/tools/24hfeed/{handle}/market_recent.json`, with broader retained posts in `{handle}/market_tweets.json` and dated snapshots under `{handle}/days/YYYY-MM-DD.json`.
   A plain `curl -L https://daytrading.monster/tools/24hfeed/` exposes the current static/noscript narrative summary and page structure without browser control. Use only the current page/current JSON state; do not fetch archives, historical windows, or old feed backfills for normal macro work. Treat both monitors as social-media-derived screening, not final macro proof; verify important current items with live macro tape, original/source reporting when available, and market prices.
...
If a market report passes in pre-screened narrative-status entries, or if you fetch DayTrading.monster 24H Feed directly, use them only as social-media-derived macro or theme candidates. Use the feed's current topic summaries and only entries whose source/update time fits the active market window. Do not mine older entries from the current JSON as historical evidence. Fresh entries may guide what to verify in live macro tape and market prices; stale entries are background and must not be treated as current macro evidence.
...
2. Fetch the minimum needed recent items from both streams:
   ...
   - Use DayTrading.monster 24H Feed as an extra current narrative pre-screen when the question is broad macro, cross-asset, pre-market, after-hours, or market-regime related. Prefer the current `narrative_status.json` topic summaries and timestamps; the HTML page's noscript current summary is also readable without browser control. Do not fetch historical/archive feed data, and do not use 24H Feed alone as final evidence. In final prose, describe this layer generically as `当前叙事预筛` or `当前主题线索`, not by site/feed/page name.
