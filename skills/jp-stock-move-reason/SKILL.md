...
- Institution rating check: after the collector run, check the target code against DayTrading.monster analyst-rating data (`https://daytrading.monster/rating/`). The page itself is a frontend shell and a plain HTML fetch only shows the loading placeholder, so prefer the public JSON files:
  - `https://daytrading.monster/rating/data.json`: rolling 30-day rating report list with `generatedAt`, `publishedDate`, `stockCode`, `stockName`, `broker`, `brokerOrigin`, `title`, `ratingText`, `targetPriceText`, `ratingSignal`, `targetSignal`, and `sourceName`.
  - `https://daytrading.monster/rating/days/index.json`: available daily files, report counts, stock counts, and broker counts.
  - `https://daytrading.monster/rating/days/YYYY-MM-DD.json`: same-day rating details when a specific report date matters.
  - `https://daytrading.monster/rating/quotes.json`: quote references for a small set of JP stocks used by the rating page.
  - `https://daytrading.monster/rating/reasons.json`: supplementary generated reasons from Yahoo Finance 掲示板 for some high-interest rating names; use only as low-weight sentiment context, not as verified fact.
  Filter by exact normalized `stockCode` and keep only reports whose `publishedDate` fits the move window or explains a recent expectation gap. If the stock appears, mention broker, date, rating direction, target-price direction, and whether the update is likely a primary catalyst, secondary support, or stale background. If the stock does not appear, say there is no recent matching rating in that feed rather than implying no analyst activity exists anywhere.
