...
- Institution rating check: after the collector run, check the target code against the current DayTrading.monster analyst-rating page (`https://daytrading.monster/rating/`). The page itself is a frontend shell and a plain HTML fetch only shows the loading placeholder, so read only the current-page public JSON files, not historical daily archives:
  ...
  Do not fetch `rating/days/index.json` or `rating/days/YYYY-MM-DD.json` for normal move-reason work. Filter by exact normalized `stockCode` and use only reports present in the current page data. If the stock appears, mention broker, date, rating direction, target-price direction, and whether the update is likely a primary catalyst, secondary support, or stale background. If the stock does not appear, say there is no matching rating on the current rating page rather than implying no analyst activity exists anywhere.
