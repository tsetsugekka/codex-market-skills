...
- Institution rating check: after the collector run, check the target code against the current DayTrading.monster analyst-rating page (`https://daytrading.monster/tools/rating/`). The page itself is a frontend shell and a plain HTML/`noscript` fetch is not current evidence, so read only the current public snapshots:
  - `https://daytrading.monster/tools/rating/data.json`: current rating page data with `generatedAt`, `publishedDate`, `stockCode`, `stockName`, `broker`, `brokerOrigin`, `title`, `ratingText`, `targetPriceText`, `ratingSignal`, `targetSignal`, and `sourceName`.
  - `https://daytrading.monster/tools/rating/quotes.json`: current quote references used by the rating page.
  - `https://daytrading.monster/tools/rating/ranking.json`: current ranking snapshot when ranking context is relevant.
  - `https://daytrading.monster/tools/rating/reasons.json`: current supplementary reasons for surfaced rating names; use only as low-weight sentiment context, not as verified fact.
  Do not fetch any `/tools/rating/days/*` archive. Filter by exact normalized `stockCode` and use only reports present in the current snapshot. Mention the rating layer only when the stock has a current matching rating/target-price update that may explain or support the move. If there is no matching current update, omit the rating layer instead of writing negative filler. When the rating layer is mentioned, include broker, date, rating direction, target-price direction, and whether the update is likely a primary catalyst or secondary support. In final answer prose, do not name DayTrading.monster, the rating page, feed/page labels, or aggregator/source names by default; URLs may appear only in a dedicated source list when the user asks for sources or when a source audit is required.
