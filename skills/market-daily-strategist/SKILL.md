...
5. Optionally run the narrative-status helper as a pre-screen for the active market when a broad report needs current theme discovery:
   ...
   The helper reads the current DayTrading.monster Market narrative snapshot (`https://daytrading.monster/24hfeed/feed24.json`). For pre-market and close-review reports, use only this current runtime snapshot when the report needs current cross-asset or theme discovery, then verify decisive items with live news, original/source reporting when available, and prices. Do not fetch or mine historical/archive 24H Feed data, daily shards, or backfilled rows for normal reports. Do not use the HTML page's `noscript` or static SEO summary as current evidence because it can lag the runtime app.
   When raw X account context would materially improve a report, use the current public account snapshot (`https://daytrading.monster/24hfeed/x-monitor-feed.json`). Do not read `accounts.json`, per-account daily shards, or legacy `market_recent.json` / `market_tweets.json`. Treat this as social-media-derived source context, not verified news.
