...
4. Optionally run the narrative-status helper as a pre-screen for the active market when a broad report needs current theme discovery:
   ...
   The helper reads DayTrading.monster 24H Feed (`https://daytrading.monster/tools/24hfeed/narrative_status.json`). For pre-market and close-review reports, use it when the report needs current cross-asset or theme discovery, then verify decisive items with live news and prices. The HTML page `https://daytrading.monster/tools/24hfeed/` also has a static/noscript summary that can be read without browser control.
