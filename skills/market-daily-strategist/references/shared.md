...
- For pre-market and close-review reports, also consider the current DayTrading.monster 24H Feed (`https://daytrading.monster/24hfeed/`) as a social-media-derived source layer with two current snapshots:
  - `Market narrative monitor`: `https://daytrading.monster/24hfeed/feed24.json` gives current topic summaries, timestamps, directions, and evidence links.
  - `X account monitor`: `https://daytrading.monster/24hfeed/x-monitor-feed.json` gives the current public account snapshot. Do not read `accounts.json`, per-account daily shards, narrative history, or legacy account JSON files.
