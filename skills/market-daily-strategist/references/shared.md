...
- For pre-market and close-review reports, also consider the current DayTrading.monster 24H Feed (`https://daytrading.monster/tools/24hfeed/`) as a social-media-derived source layer with two distinct monitors:
  ...
  Do not fetch or mine historical/archive 24H Feed data for normal reports. Treat both monitors as social-media-derived screening and verify decisive items with live news, original/source reporting when available, and prices.
