...
- For limit-up, after-hours, and sharp-move valuation, map the catalyst into `confirmed earnings/order`, `capital policy`, `theme premium`, or `forum/social speculation`; use a lower deserved PE for speculation-only moves and require explicit execution milestones before moving from base to bull scenarios.
