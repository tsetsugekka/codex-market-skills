...
- `--comments 100`：一页Yahoo掲示板最多缓存100条评论；筛选、评分和去重后，最终只展示5条用于归纳原因。
- `--comments 0`：完全跳过 Yahoo 掲示板请求，不只是把输出限制为0条。
- `--forum-only`：仅抓取Yahoo掲示板；榜单原因的主流程使用此模式，每只股票只抓一页、最多缓存100条评论，并只输出最终评分最高的5条。
...
掲示板：每只入榜股票只抓一页、最多缓存100条评论，依次请求，不重复抓同一代码。所有个股原因采集与榜单入榜股票使用同一条处理链：先用24小时窗口，窗口内不足100条才扩大到72小时；去掉点赞少于5的评论，按时间、点赞、长度和信息关键词评分并去重，先保留最多20条，再按时间和点赞重新排序，只展示前5条归纳原因。
