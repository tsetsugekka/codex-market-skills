...
- `--hours 24`：新闻时间窗口；Yahoo评论固定先看24小时，24小时内不足100条才扩到72小时。
- `--comments 100`：一页Yahoo掲示板最多缓存100条评论；筛选、评分和去重后，只向 Codex 提供5条作为内部归纳原因的输入。
- `--comments 0`：完全跳过 Yahoo 掲示板请求，不只是把输出限制为0条。
- `--forum-only`：仅抓取Yahoo掲示板；榜单原因的主流程使用此模式，每只股票只抓一页、最多缓存100条评论，并只向 Codex 提供最终筛选出的5条作为原因输入。
...
掲示板：每只入榜股票只抓一页、最多缓存100条评论，依次请求，不重复抓同一代码。所有个股原因采集与榜单入榜股票使用同一条处理链：先用24小时窗口，窗口内不足100条才扩大到72小时；去掉点赞少于5的评论，按时间、点赞、长度和信息关键词评分并去重，先保留最多20条，再按时间和点赞重新排序，只把前5条提供给 Codex 归纳原因。用户最终只看归纳后的`原因`，除非明确要求查看原帖。
