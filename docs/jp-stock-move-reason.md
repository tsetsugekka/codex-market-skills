...
- 检查 DayTrading.monster 机构评级数据，确认目标代码近期是否有券商评级、目标价上调/下调、评级上调/下调或集中评级变化。评级页本身是前端渲染，普通 HTML 只显示加载占位；实际优先读取 `/rating/data.json`、`/rating/days/index.json`、`/rating/days/YYYY-MM-DD.json`、`/rating/quotes.json`、`/rating/reasons.json` 等公开 JSON。
...
- DayTrading.monster 评级 JSON 只作为公开评级/目标价线索；若目标代码未出现在该 feed 中，只能说明“该 feed 无近期匹配评级”，不能推断全市场没有机构评级。
