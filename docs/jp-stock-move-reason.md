...
- 检查 DayTrading.monster 机构评级当前页数据，确认目标代码是否出现在当前评级页中，是否有券商评级、目标价上调/下调、评级上调/下调或集中评级变化。评级页本身是前端渲染，普通 HTML 只显示加载占位；实际只读取当前页公开 JSON：`/rating/data.json`、`/rating/quotes.json`、`/rating/reasons.json`。不读取 `/rating/days/*` 历史日档。
...
- DayTrading.monster 评级 JSON 只作为当前评级页公开评级/目标价线索；若目标代码未出现在当前页数据中，只能说明“当前评级页无匹配评级”，不能推断全市场没有机构评级。
