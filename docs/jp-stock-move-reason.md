...
- 按日本交易时段自动选择 Kabutan 日中或夜间 PTS 涨跌榜，筛选有效异动后计算 PTS 出来额 Top10，并逐只补充原因。
...
它用于处理“跑一下 PTS”“PTS 涨跌 Top10”“夜间 PTS 成交额榜”等请求。
默认同时抓取上涨榜和下跌榜，并按以下规则执行。

### 时间与 URL 路由

`--session auto` 按日本时间选择 Kabutan section：

- 交易日 `08:20-15:30` 日中取引：使用 day section。
- 交易日 `15:30-16:30` 大引け後：仍使用 day section。
- `17:00-06:00` 夜間取引：使用 night section。
- 其他时段、周末和非交易日：使用 night section。
...
因此 `06:00-08:20`、`16:30-17:00` 以及夜间交易结束后的其他非日中时段
也读取 night section。判定必须使用 JST，不能使用运行机器的其他本地时区。

对应页面：

- 日中上涨：`https://kabutan.jp/warning/pts_day_price_increase`
- 日中下跌：`https://kabutan.jp/warning/pts_day_price_decrease`
...
Kabutan 榜单不一定直接提供出来额，因此按以下公式估算：

```text
PTS出来额 = PTS价格 * PTS出来高
```

最终按计算出的 PTS 出来额降序取上涨、下跌各 Top10。`PTS出来高`只作为
`> 2,000` 的资格条件，默认不能误作排名指标；只有用户明确要求“按出来高排序”
时才改用原始出来高。
...
日中或夜间 section、页面时间戳和计算口径写在标题下方，不改动标题文字。
...
Kabutan 页面以涨跌幅排序，脚本使用每页 50 条并继续翻页，直到页面越过 3%
阈值，避免只看第一页漏掉成交额较大的股票。

### 抓取规则与规范

- 使用 Kabutan 当前 day/night warning 页面，并通过 `shared_perpage=50` Cookie
  读取每页 50 条；不把可能滞后的 `noscript`、SEO 静态内容当作当前榜单。
- 请求带 cache-busting 时间参数，解析页面显示的 `YYYY年MM月DD日 HH:MM現在`
  并在结果中报告该时间；Kabutan PTS 通常约有15分钟延迟，不能称为逐笔实时数据。
- 上涨榜按页面顺序抓取，直到最后一条低于 `+3%`；下跌榜抓取到最后一条高于
  `-3%`。空页立即停止，并设置最大页数防止异常循环。
- 完成涨跌幅范围采集后，才应用 `PTS出来高 > 2,000`，再计算出来额并排序；
  不能按页面原顺序或原始出来高直接取 Top10。
- 日中 `08:20-15:29` 的涨跌率相对前一个正规交易时段收盘价；`15:30` 后的
  day section 以及夜间 section 相对当日正规市场收盘价。输出时不要混淆比较基准。
...
最终榜单默认必须包含 `原因` 列，除非用户明确只要数字。普通股票使用
`stock_move_sources.py` 顺序核对新闻、公司披露、评级和 Yahoo 掲示板；优先采用
硬催化，掲示板只作为市场关注点和传闻检查。连续请求同一站点时保持适度等待，
不高频批量抓取全部掲示板。ETF/ETN 则根据其跟踪指数或策略解释，低成交额异动
应明确标为低置信度。
