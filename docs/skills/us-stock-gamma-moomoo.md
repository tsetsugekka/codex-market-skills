...
`us-stock-gamma-moomoo` 是一个用于 Codex 的美股期权 gamma 分析 skill。它通过本机 moomoo OpenD 获取美股和期权数据，再由 Codex 输出 gamma/GEX、gamma wall、gamma flip、SPX/SPY/ES 盘中结构和 0DTE 期权情景表。
...
这个 skill 可以放在公开仓库中。请不要提交 moomoo 账号、OpenD 日志、个人截图、私有行情输出、`.env`、token 或任何含有登录信息的文件。
