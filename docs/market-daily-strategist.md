...
- DTM 接入优先遵循 `https://daytrading.monster/api-docs/`：`/api/themes` 用于 US/JP/CN 跨市场题材传导；日本 PTS 整体上涨优先 `/api/pts/model1`、`model2`、`model3`，其他日股榜单及下跌查询参考 Kabutan/Yahoo 榜单入口；A 股用 `/api/chinastock-anomaly`，指数背景按需用 `/api/gamma/`、`/api/range/nikkei`、`/api/range/sse`。本策略 Skill 不请求两个 DTM ratings API。
