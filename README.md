...
统一处理 A 股盘中/盘后的题材强弱、板块主力资金流、涨停池和机构调研。题材模块读取本地题材映射和中文标签，用东方财富妙想行情按映射权重计算 TOP10/BOTTOM10；其他模块优先使用妙想聚合数据，字段不支持或不完整时切换到公开聚合备用源，并报告数据时间、口径和来源变化。板块资金流还支持当天分钟级累计净流入折线图，保留交易时段空档并标注零轴和资金拐点。
