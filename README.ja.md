...
<p align="center">
  ...
  <img alt="Language" src="https://img.shields.io/badge/Language-日本語-blue">
...
- 下記の単体インストール：Codex で特定の Skill や付属のローカルスクリプトを使う場合。必要条件は各 Skill に記載しています。
...
| [`jp-stock-move-reason`](docs/jp-stock-move-reason.md) | 日本株の変動理由を分析し、時間帯別に現物市場または PTS の売買代金 Top10 を作成 | なし |
...
| 「現在の日本株上昇・下落 Top10 を推計売買代金順で理由付き表示して。」 | `jp-stock-move-reason` |
...
売買代金ランキングの副 skill も含みます。取引日は JST `09:00-11:30` と `12:30-15:30` に Kabutan の通常騰落率ページ、`08:00-09:00`、`11:30-12:30`、`15:30-17:00` に PTS day、その他の時間帯と非取引日に PTS night を使用します。1ページ50件で、騰落率の絶対値が3%以上かつ出来高が2,000株超の銘柄を対象に、価格と出来高から推計売買代金を計算し、上昇・下落 Top10 を理由付きで出力します。理由の一括収集では Yahoo にアクセスせず、原因が不明な銘柄に限って最大2銘柄まで掲示板を低頻度で補足確認します。
...
- 現在の現物市場または PTS の上昇・下落銘柄を、出来高ではなく推計売買代金でランキングする。
...
- 盤中/引け後にテーマ強弱 TOP10/BOTTOM10 を計算する。
- `主力净流入 Top10` と `主力净流出 Top10` を固定形式で表示する。
- 涨停数、連板梯子、炸板、業種/テーマ分布を確認する。
- 現在または過去の機関調査熱度を確認し、非対応の日付や項目は明記する。
- デフォルトではファイルを書かず、代替ソースの制限・不安定時には host、endpoint family、エラーを報告する。
...
1 つだけ必要な場合は、`stock-technical-analysis` や `cn-market-tape` のように skill 名も指定します。
